...
{
	float d=1024.0;
	float t = 1*object_speed_modifier+5.0;
...
{
	float t = 1*.5*object_speed_modifier + 2.0;
	...
	{
		dd = dist(p);
        noncaptured = smoothstep(0.0, 0.666, dd);
		p+=d*dd*.7;
	...
	md = normalize(md);
	md = rotate_x(md,0.5*9.0+offset1);
	md = rotate_y(md,0.5*9.0+offset2);
	...
	color -= length(uv)*.1;
	color =cc(color,.5,.6);
    color += pow(texture(DiffuseSampler, texCoord).rgb,vec3(1.0)) * noncaptured;
