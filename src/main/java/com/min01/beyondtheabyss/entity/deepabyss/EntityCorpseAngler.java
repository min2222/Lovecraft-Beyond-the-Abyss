...
public class EntityCorpseAngler extends AbstractDeepAbyssMonster
{
	...
	public void tick() 
	{
		...
		{
			BTAUtil.animateWhen(this.idleAnimationState, this.isInWater() && !BTAUtil.isMoving(this), this.tickCount);
		}
		if(this.getAnimationState() == 0 && this.isInWater())
		{
			if(BTAUtil.isCollisionShapeFullBlock(this.level, this.blockPosition().below()) && BTAUtil.isCollisionShapeFullBlock(this.level, this.blockPosition().below(2)) && BTAUtil.isCollisionShapeFullBlock(this.level, this.blockPosition().below(3)))
			{
