...
public class PutridBubbleEntity extends ThrowableProjectile
{
	public static final EntityDataAccessor<Boolean> IS_EXPLODE = SynchedEntityData.defineId(PutridBubbleEntity.class, EntityDataSerializers.BOOLEAN);
	
	public int explosionTick;
	
	public PutridBubbleEntity(EntityType<? extends PutridBubbleEntity> pEntityType, Level pLevel) 
	{
		super(pEntityType, pLevel);
		this.setNoGravity(true);
	}

	public PutridBubbleEntity(Level pLevel, LivingEntity pShooter) 
	{
		super(BTAEntities.PUTRID_BUBBLE.get(), pShooter, pLevel);
		this.setNoGravity(true);
	}

	public PutridBubbleEntity(Level pLevel, double pX, double pY, double pZ)
	{
	...
	public void tick()
	{
		...
		if(this.isExplode() || this.tickCount >= 100)
		{
			...
			List<LivingEntity> list = this.level.getEntitiesOfClass(LivingEntity.class, this.getBoundingBox().inflate(1.5F), t -> 
			{
				...
				return !(t instanceof MutavoreEntity) && flag;
			});
			list.forEach(t -> 
			{
