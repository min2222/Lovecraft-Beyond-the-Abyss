...
//https://github.com/AlexModGuy/AlexsCaves/blob/main/src/main/java/com/github/alexmodguy/alexscaves/server/level/structure/AbstractCaveGenerationStructure.java
public abstract class AbstractCaveGenerationStructure extends Structure 
{
    ...
    {
        return this.atYCaveBiomePoint(context, Heightmap.Types.OCEAN_FLOOR_WG, (builder) -> 
        {
    ...
    public void generatePieces(StructurePiecesBuilder builder, Structure.GenerationContext context) 
    {
        ...
        int biomeUp = this.biomeContinuesInDirectionFor(context.biomeSource(), context.randomState(), Direction.UP, center, heightRad) + this.getYExpandUp();
        int biomeDown = this.biomeContinuesInDirectionFor(context.biomeSource(), context.randomState(), Direction.DOWN, center, heightRad) + this.getYExpandDown();
        ...
        int widthChunks = (int) Math.ceil((widthBlocks + 16) / 16.0F / 2.0F) + 2;
        int heightChunks = this.getHeightOverride((int) Math.ceil((heightBlocks + 16) / 16.0F / 2.0F));
        int minYChunks = (int) Math.ceil(context.chunkGenerator().getMinY() / 16.0F);
        int maxYChunks = (int) Math.ceil(320 / 16.0F);
