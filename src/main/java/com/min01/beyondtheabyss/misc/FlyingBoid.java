...
public class FlyingBoid
{
	public final Mob mob;
	public final List<FlyingBoid> boids = new ArrayList<>();
	public List<? extends Mob> nearbyMobs = new ArrayList<>();
	
	public Vec3 velocity = Vec3.ZERO;
    public Vec3 target = Vec3.ZERO;

	public FlyingBoid(Mob mob)
	{
		this.mob = mob;
		this.velocity = new Vec3(Math.random(), Math.random(), Math.random());
	}

	public void update(Collection<FlyingBoid.Obstacle> obstacles, boolean avoidance, boolean alignment, boolean cohesion, float flockRadius, float maxVelocity) 
	{
		this.tickNearbyMobs();
		
		Collection<FlyingBoid> boids = this.boids;
		
		Collection<FlyingBoid> flock = this.getInRange(boids, this.mob.position(), flockRadius);
		...
		if(avoidance)
		{
			...
			for(FlyingBoid boid : flock)
			{
				flockObstacles.add(new FlyingBoid.Obstacle(boid.mob.position(), 0.5, 0.05));
			}
    ...
    public void tickNearbyMobs()
    {
        ...
        {
        	if(mob.getMoveControl() instanceof FlyingBoidMoveControl control)
        	{
            	FlyingBoid boid = control.flyingBoid;
        ...
        if(!this.nearbyMobs.isEmpty() && this.nearbyMobs.get(0).getMoveControl() instanceof FlyingBoidMoveControl boid)
        {
	...
	private Vec3 averageVelocity(Collection<FlyingBoid> flock)
	{
		Vec3 avg = Vec3.ZERO;
		if(flock.size() == 0)
		{
	...
	private Vec3 centerDisplacement(Collection<FlyingBoid> flock)
	{
	...
	private Collection<FlyingBoid> getInRange(Collection<? extends FlyingBoid> boids, Vec3 location, double radius)
	{
		Collection<FlyingBoid> out = new ArrayList<FlyingBoid>();
		for(FlyingBoid other : boids) 
		{
